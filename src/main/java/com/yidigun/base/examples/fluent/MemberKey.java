...
/// ## [Aware.Builder] 인터페이스
/// 
/// [Aware] 객체 생성에 사용할 Builder 용 인터페이스이다.
///
...
///
/// [Aware.Builder#memberKey(MemberKey)]가 기본 메소드로 구현되어있어
...
/// ```
/// 
/// ### [Aware.Builder]가 제네릭 타입을 사용하는 이유
/// 
...
/// * 그렇지만, [Aware.Builder] 인터페이스를 통해서
///   [Aware] 객체가 생성될 때는 `memberNo`를 `0`으로 설정할 수 있는데,
///   이는 객체가 DB에 저장되어 PK값이 생성되기 전의 상황이거나
///   PK가 아닌 `memberNo` 필드의 값이 `null`임을 의미한다.
/// * 마찬가지로 [Aware#memberKey()] 메소드는 `memberNo <= 0`인 경우 `null`을 리턴한다.
///
/// @see PrimaryKey
/// @see Member
/// @see Address
...
public final class MemberKey implements PrimaryKey, Comparable<MemberKey> {
    ...
    public interface Aware {
        ...
        /// `null` 확인을 피하려면 [#memberKeyNullable()]를 사용한다.
        ///
        /// @return MemberKey 객체 또는 null
        default MemberKey memberKey() {
            return tryOf(memberNo()).orElse(null);
        }

        /// `null` 검사를 피하기 위해 사용할 수 있는 [MemberKey] 조회 메소드.
        ///
        /// @return MemberKey 객체를 감싼 Optional 객체
        default Optional<MemberKey> memberKeyNullable() {
            return tryOf(memberNo());
        }
        ...
        default boolean hasValidMemberKey() {
            return memberNo() > NULL_MEMBER_NO;
        }
        ...
        interface Builder<B extends Builder<B>> {
            ...
            /// [Aware#memberKeyNullable()]를 사용하여 조회한 값을
